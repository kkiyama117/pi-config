...
| task-101 | DENYLIST structural enforcement in `verify_repo` (immutable policy, fail-closed scans, bootstrap snapshot, review-diff emission helpers) | 7 reviewer rounds folded (replace-refs hardening, reviewer worktree-edit guard, merge-commit diffs, stat-only large files, strict final-line PASS); `tests/test_denylist.sh` 156 PASS. Loop-branch merge into the v2 branch was the last missing step |
| task-102 | `STAGE_TIMEOUT_S` default 0 → 900 (0 = disabled, `timeout 0` semantics); timeout kill (rc 124) logs `pi attempt model=<m> TIMEOUT after <N>s stage=<s>` in addition to the took line | No control-flow change; the normal failure path (usage salvage → fallback) handles the kill. `tests/test_stage_timeout.sh` (fake-pi stub: sleeping primary + instant fallback, 0-disables case) |
| task-103 | Budget guards: `MAX_RUN_COST_USD` (2.0), `MAX_RUN_TOKENS` (3000000), `BUDGET_WARN_PCT` (80) — warn once per budget type + notify, die at ≥100%; EXIT trap still emits the cost summary | Stop-and-ask decision: the guard fires from `record_usage_if_any`'s salvage path too — the original failure is already logged there, so the budget stop is the correct hard stop (no masking). Token guard is the real backstop (cursor reports cost 0). End-to-end stub tests (`test_budget_guard.sh`) caught a test-harness bug: `export PATH` inside a command substitution is lost — harness setup must not run in a subshell (also cost a real 0.06 USD pi run before the fix) |
| task-104 | `LOOP.md` stop-conditions charter + `tests/test_loop_md_sync.sh` (6 keys, both drift directions self-tested) wired into the `~/.pi` VERIFIERS gate | Sync extraction must strip trailing comments from the `VAR="${VAR:-default}"` line (`s/}".*//p`, not `}"$//p`). No parser: loop.sh never reads LOOP.md |
| task-105 | `metrics.py` (stdlib): `summary` (per-run + done/review-fail rates, escalated count), `check-cost` (≤ N× median of prior done runs, ≥2 prior or no-baseline exit 0), `theater` (rate vs --max-pct, default 30) | Escalation proxy: no dedicated escalation record exists, so escalated = runs with verify-fail/review-fail entries (escalate_worker fires on both) — documented in the docstring. Malformed lines skipped, never fatal. Loop.sh untouched |
| task-106 | GATE-2 approve path removes the worktree (`worktree removed; branch <b> kept — merge with: git merge <b>`); "worktree left at" only when it still exists; reject/rollback and dry-run unchanged | Branch kept for the manual merge to develop. Full-run stub test (`test_worktree_cleanup.sh`) proves approve (removed, branch kept with commit) and reject (removed, branch dropped); `git branch --list` output needs whitespace stripping before use as a ref |
